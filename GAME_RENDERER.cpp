...
namespace GAME_RENDERER
{
	...
	bool GAME_RENDERER::isDeviceSuitable(VkPhysicalDevice device)
	{
		...
		return deviceProperties.deviceType == VK_PHYSICAL_DEVICE_TYPE_DISCRETE_GPU &&
			   deviceMemory.memoryHeaps->size > 1073741824 && 
	...
	void GAME_RENDERER::createLogicalDevice()
	{
		...
		if (vkCreateDevice(physicalDevice, &createInfo, nullptr, &device) == VK_SUCCESS) {
			STD_OUTPUT << "LOGICAL DEVICE CREATED!" << STD_ENDL;
		}
		vkGetDeviceQueue(device, indices.graphicsFamily.value(), 0, &graphicsQueue); // получение дескриптора очереди графики
		vkGetDeviceQueue(device, indices.presentFamily.value(), 0, &presentQueue); // получение дескриптора очереди показа
	}

	void GAME_RENDERER::createSurface()
	{
		if (glfwCreateWindowSurface(instance, mainWindow, nullptr, &surface) == VK_SUCCESS) {
			STD_OUTPUT << "SURFACE CREATED!" << STD_ENDL;
		}
	}

	bool GAME_RENDERER::checkDeviceExtensionSupport(VkPhysicalDevice device)
	{
	...
	void GAME_RENDERER::CreateSwapChain()
	{
	...
	SwapChainSupportDetails GAME_RENDERER::querySwapChainSupport(VkPhysicalDevice device)
	{	
	...
	VkSurfaceFormatKHR GAME_RENDERER::chooseSwapSurfaceFormat(const std::vector<VkSurfaceFormatKHR>& availableFormats)
	{
	...
	VkPresentModeKHR GAME_RENDERER::chooseSwapPresentMode(const std::vector<VkPresentModeKHR>& availablePresentModes)
	{
	...
	VkExtent2D GAME_RENDERER::chooseSwapExtent(const VkSurfaceCapabilitiesKHR& capabilities)
	{
	...
	void GAME_RENDERER::createImageViews()
	{
		...
		for (size_t i = 0; i < swapChainImages.size(); i++) {
			...
			if (vkCreateImageView(device, &createInfo, nullptr, &swapChainImageViews[i]) == VK_SUCCESS) {
				STD_OUTPUT << "CREATE IMAGE VIEW!\n";
			}
		}
	}

	void GAME_RENDERER::createGraphicsPipeline()
	{
		...
		vertexInputInfo.pVertexBindingDescriptions = &bindingDescription; // Привязка струк
		vertexInputInfo.vertexAttributeDescriptionCount = static_cast<uint32_t>(attributeDescriptions.size());
		vertexInputInfo.pVertexAttributeDescriptions = attributeDescriptions.data(); // Optional

		VkPipelineInputAssemblyStateCreateInfo inputAssembly{};  // Настройка топологии
		...
		if (vkCreatePipelineLayout(device, &pipelineLayoutInfo, nullptr, &pipelineLayout) == VK_SUCCESS) {
			STD_OUTPUT << "PIPELINE LAYOUT CREATED!\n";
		}
		...
		if (vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &graphicsPipeline) == VK_SUCCESS) {
			STD_OUTPUT << "create graphics pipeline!\n";
		}
		vkDestroyShaderModule(device, fragShaderModule, nullptr);
		vkDestroyShaderModule(device, vertShaderModule, nullptr);
	}

	VkShaderModule GAME_RENDERER::createShaderModule(const std::vector<char>& code)
	{
		...
		if (vkCreateShaderModule(device, &createInfo, nullptr, &shaderModule) == VK_SUCCESS) {
			STD_OUTPUT << "SHADER MODULE CREATED!\n";
;		}
		return shaderModule;
	}

	void GAME_RENDERER::createRenderPass()
	{
		...
		VkSubpassDescription subpass{}; // подпроход рендера
		...
		renderPassInfo.pSubpasses = &subpass; // привязка подпроходов
		...
		if (vkCreateRenderPass(device, &renderPassInfo, nullptr, &renderPass) == VK_SUCCESS) {
			STD_OUTPUT << "RENDER PASS CREATED!\n";
		}
	}

	void GAME_RENDERER::createFrameBuffers()
	{
		...
		for (size_t i = 0; i < swapChainImageViews.size(); ++i) {
			...
			if (vkCreateFramebuffer(device, &framebufferInfo, nullptr, &swapChainFramebuffers[i]) == VK_SUCCESS) {
				STD_OUTPUT << " FRAMEBUFFER " << i << " CREATED!\n";
			}
		}
	}

	void GAME_RENDERER::createCommandPool()
	{
		...
		poolInfo.queueFamilyIndex = queueFamilyIndices.graphicsFamily.value(); // записывать команды для рисования, 
																			   // поэтому мы выбрали семейство графических очередей

		if (vkCreateCommandPool(device, &poolInfo, nullptr, &commandPool) == VK_SUCCESS) {
			STD_OUTPUT << " COMMAND POOL CREATED!\n ";
		}
	}
	...
	void GAME_RENDERER::createDescriptorSetLayout()
	{
	...
	void GAME_RENDERER::createDescriptorPool()
	{
	...
	void GAME_RENDERER::createDescriptorSets()
	{
	...
	void GAME_RENDERER::createUniformBuffers()
	{
	...
	void GAME_RENDERER::updateUniformBuffer(uint32_t currentImage)
	{
		...
		ubo.scale = glm::scale(glm::mat4(1.0f), glm::vec3(static_cast<float>(xpos/100) *  1.0f,1.0f,1.0f));
		//ubo.proj[1][1] *= -2;
	...
	{	
		...
		if (vkAllocateCommandBuffers(device, &allocInfo, commandBuffers.data()) == VK_SUCCESS) {
			STD_OUTPUT << " COMMAND POOL ALLOCATED!\n ";
		}
	}

	void GAME_RENDERER::recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex)
	{
		VkCommandBufferBeginInfo beginInfo{};
		beginInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO;
		...
		VkBuffer vertexBuffers[] = { vertexBuffer }; // Промежуточный буфер из глобального
	...
	void GAME_RENDERER::drawFrame()
	{
		...
		/*
			Рендеринг кадра в Vulkan состоит из общего набора шагов:
			Дождитесь окончания предыдущего кадра
			Получить image из swap chain
			Запишите командный буфер, который рисует сцену на этом изображении.
			Отправить записанный командный буфер
			Показать image цепочки обмена
		*/
		
		uint32_t imageIndex;
		
		VkResult result = vkAcquireNextImageKHR(device, swapChain, UINT64_MAX, imageAvailableSemaphores[currentFrame], VK_NULL_HANDLE, &imageIndex);
		//STD_OUTPUT << imageIndex << " index. " << currentFrame << " current frame\n";
		...
		//STD_OUTPUT << "DRAW FRAME " << currentFrame << STD_ENDL
		using namespace std::chrono_literals;
		//std::this_thread::sleep_for(100ms);
	}

	void GAME_RENDERER::createSyncObjects()
	{
	...
	void GAME_RENDERER::recreateSwapChain()
	{
	...
	void GAME_RENDERER::cleanupSwapChain()
	{
	...
	std::vector<char> GAME_RENDERER::readFile(const std::string& filename)
	{
